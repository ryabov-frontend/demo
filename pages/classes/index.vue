    ...
    .site-top
      ...
      main.site-main
        ...
        .widget.widget-classes
          .container(
            v-for = "block in classes.categories"
            :key = "block.name")

            h3.text-center.title.m-t-30.m-b-30(v-html = "block.name")

            .text(v-html = "block.description")

            .master_class-list-new
              .master_class-new.classes(
                v-for = "(item, i) in block.classes"
                :key = "item.id")
...
export default {
  async asyncData({ $axios }) {
    ...
    const data = await $axios.$get(
      '/api/site/get-page-info-by-slug?slug=classes'
    )
    const classes = await $axios.$get("/api/calendar/get-classes")
    return { options, menu, data, classes }
  },
  data: () => ({
    meta: {
      title: "",
